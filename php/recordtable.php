  ...
  <body>
    ...
    <table class="styled-table" id= "table_id">
      ...
      <tbody id="tbody_table_record">
        <?php
          ...
          echo '<h3 style="color: #0c6980;">' .$sensorName. '</h3>';
          include 'database.php';
          $num = 0;
          $pdo = Database::connect();
          $sql = 'SELECT * FROM esp32_record where sensorName = "' . $sensorName . '" ORDER BY date desc, time desc, sensorName';
          foreach ($pdo->query($sql) as $row) {
            ...
            echo '<tr>';
            echo '<td>'. $dateFormat . " " . $timeFormat . '</td>';
            echo '<td class="bdr">'. $row['temperature'] . '</td>';
            echo '<td class="bdr">'. $row['humidity'] . '</td>';
            echo '<td class="bdr">'. $row['battery'] . '</td>';
            echo '<td class="bdr">'. $row['RSSI'] . '</td>';
            echo '</tr>';
          }
          Database::disconnect();
          //------------------------------------------------------------
    ...
    <div class="btn-group">
      ...
      <div style="display: inline-block; position:relative; border: 0px solid #e3e3e3; float: center; margin-left: 2px;;">
        <p style="position:relative; font-size: 14px;"> Pagina : <span id="page"></span></p>
      </div>
    </div>
    <div class="btn-group">
      ...
      <div style="display: inline-block; position:relative; border: 0px solid #e3e3e3; float: center; margin-left: 2px;;">
        <p style="position:relative; font-size: 14px;"> Righe per pagina <span id="nr"></span></p>
      </div>
    </div>
